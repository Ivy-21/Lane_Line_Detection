...
for lane in gt_lanes_vis:
    cv2.polylines(img_vis, np.int32([lane]), isClosed=True, color=(0,255,0), thickness=2)
...
print("mask_img_dilation : ", mask_img_dilation.shape)
...
# ####################################################################################
# replaced_image = cv2.bitwise_or(img,img,mask = mask_img_dilation)



# ###############################################Erosion#############################################
# kernel = np.ones((7, 7), np.uint8)
# mask_img_erosion = cv2.erode(mask_img_dilation, kernel, iterations=1)

# cv2.imshow('Mask Erosion', mask_img_erosion)
# cv2.waitKey(0)
# cv2.destroyAllWindows()
